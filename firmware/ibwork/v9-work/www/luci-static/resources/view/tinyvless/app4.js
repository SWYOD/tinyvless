...
var STYLE = [
	...
	'.tv-grid{ display:grid; grid-template-columns:1fr; gap:16px; align-items:start; }',
...
return view.extend({
	...
	render: function (data) {
		...
		var st = {
			...
			memTotal: 0, memAvail: 0, load1: 0, uptime: 0, flashPct: 0
		};
		...
		try {
			...
			if (js.udp_tunnel) st.udpTunnel = js.udp_tunnel;
			st.memTotal = js.mem_total || 0; st.memAvail = js.mem_avail || 0;
			st.load1 = js.load_1 || 0; st.uptime = js.uptime || 0; st.flashPct = js.flash_pct || 0;
		} catch (e) {}
		...
		function doPoweroff() {
			...
			ui.hideModal();
			setBusy(true);
			bannerShow('Выключаю роутер…');
			fs.exec('/etc/tinyvless/api.sh', ['poweroff']).then(function (r) {
				if (!/"ok":true/.test(r.stdout || '')) throw new Error('bad');
				bannerOk('Роутер выключается — включите питание снова');
			}).catch(function () { setBusy(false); bannerErr('Не удалось выключить'); });
		}
		...
		function doReboot() {
			...
			ui.hideModal();
			setBusy(true);
			bannerShow('Перезагружаю роутер…');
			fs.exec('/etc/tinyvless/api.sh', ['reboot']).then(function (r) {
				if (!/"ok":true/.test(r.stdout || '')) throw new Error('bad');
				bannerOk('Перезагрузка — страница переподключится через ~1–2 мин');
			}).catch(function () { setBusy(false); bannerErr('Не удалось перезагрузить'); });
		}
		...
		function renderSysinfo() {
			...
			var load = st.load1 / 65536;
			elCpuTxt.textContent = load.toFixed(2) + ' (1 мин)';
			elCpuGauge.innerHTML = ''; elCpuGauge.appendChild(mkGauge(Math.min(load / 2 * 100, 100)));
			elUpTxt.textContent = fmtUptime(st.uptime);
			elFlashTxt.textContent = st.flashPct ? st.flashPct + '% overlay' : '—';
			elRuTxt.textContent = st.ruSet ? (st.ruSubnets + ' подсетей (/16 agg.)') : 'выкл (только домены)';
		}
		...
		var statusBox = E('div', { 'class': 'tv-status' }, [
			...
			E('div', { 'class': 'tv-stat-grid' }, [
				...
				mkStat('CPU load', [elCpuTxt, elCpuGauge]),
		...
		function applyTuning(key, val, onFail) {
			...
			setBusy(true); bannerShow('Применяю настройку…');
			fs.exec('/etc/tinyvless/api.sh', ['tuning', key, val]).then(function (r) {
				...
				setBusy(false); bannerOk('Настройка применена');
			}).catch(function () { if (onFail) onFail(); setBusy(false); bannerErr('Не удалось применить'); });
		}
		...
		var swUdpDiscord = E('input', { type: 'checkbox' });
		swUdpDiscord.checked = st.udpTunnel === 'discord';
		swUdpDiscord.addEventListener('change', function () {
			var disc = swUdpDiscord.checked, prev = st.udpTunnel;
			st.udpTunnel = disc ? 'discord' : 'full';
			applyTuning('udp_tunnel', st.udpTunnel, function () { swUdpDiscord.checked = !disc; st.udpTunnel = prev; });
		});
		...
		var controlCard = E('div', { 'class': 'tv-card' }, [
			...
			mkTrow('UDP только Discord', 'Вкл: tproxy UDP только Discord-порты. Выкл: весь UDP через tproxy.', swUdpDiscord)
		]);
		...
		return E('div', { 'class': 'tv' }, [
			E('style', {}, STYLE),
			E('div', { 'class': 'tv-wrap' }, [
				E('h2', { style: 'display:flex;align-items:center;gap:9px;margin:0 0 16px' }, [ic('shield', 26), 'tinyvless — VLESS-роутер']),
				elBanner,
				E('div', { 'class': 'tv-grid' }, [statusBox, controlCard, profCard, domCard, dnsCard, systemCard])
			])
		]);
	},
