#!/bin/bash
# V13 MagnumOpusPlus — публичный дистрибутивный релиз поверх V11: все бинари (tinyvless, tvled,
...
IB_NAME="tinyvless-v13-magnumopusplus"
...
# lt300-tinyvless-${TAG} (MagnumOpusPlus V13 — net-watchdog + AT-порт flock)
...
- Все бинари: tinyvless, tvled, atcmd — atcmd и tvled пересобраны (общий flock на AT-порт,
  настраиваемый таймаут AT-команды и интервал опроса tvled через microtun.conf)
- Изменения относительно V12:
  - **net-watchdog** — новый демон: периодически опрашивает модем + здоровье сети провайдера
    (ping+nslookup к настраиваемому домену), пишет состояние в /tmp/.tv_netcheck.json
  - **tvled и net-watchdog делят один AT-порт через flock** (\`/tmp/.tv_atport.lock\`) — раньше
    ничего не защищало параллельный доступ к \`/dev/ttyUSB*\`
  - Карточка «Модем»: новые поля «Здоровье сети»/«Проверка была», кнопка «Обновить светодиоды»
  - Баннер плохой связи (2 состояния: слабый сигнал / нет ответа) вверху основной панели,
    опциональный
  - Страница «Для разработчиков»: видимость полей/кнопок карточки «Модем» настраивается
  - Микротюнинг: интервалы опроса, таймаут AT-команды, домен для health-чека — новый блок
  - Мигание переднего LED (белый/красный поочерёдно) при старте poweroff/reboot из панели
- Отличия от личной сборки юзера (унаследовано от V12): root-пароль не задан, WiFi открыт без
  пароля, LAN закреплён на 192.168.10.1/24, VLESS_LINK пуст.
...
for try in 1 2 3 4; do
  make image FORCE=1 \
    ...
    >/out/build-v13-magnumopusplus.log 2>&1
  RC=\$?
  [ \$RC -eq 0 ] && break
  sleep 5
done
set -e
[ \$RC -eq 0 ] || { tail -40 /out/build-v13-magnumopusplus.log; exit \$RC; }
