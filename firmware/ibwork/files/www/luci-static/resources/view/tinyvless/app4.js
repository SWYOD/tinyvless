...
// ============ tinyvless морда v3 ============
// Карточки: Управление (вкл/выкл сервиса, 3 режима, автозагрузка) · Профили · Статус · Домены.
...
return view.extend({
	...
	render: function (data) {
		...
		var st = { rawCfg: data[0] || '', mode: cfg.mode, running: false, autostart: false, profiles: [], ruSubnets: 0 };

		// state из api.sh
		try { var js = JSON.parse(data[4] || '{}'); st.running = !!js.running; st.autostart = !!js.autostart; st.ruSubnets = js.ru_subnets || 0; } catch (e) {}
