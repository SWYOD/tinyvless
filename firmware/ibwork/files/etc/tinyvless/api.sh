...
case "$1" in
	status)
		...
		;;
	state)
		# сводка для инициализации морды (running + autostart + режим + размер встроенной RU-базы)
		...
		echo "{\"running\":$run,\"autostart\":$(is_autostart),\"mode\":\"$mode\",\"ru_subnets\":$ru,\"zapret\":$zap,\"nfqws\":$nfq}"
		;;
	mode)
		...
		set_conf MODE "$m"
		setsid /etc/tinyvless/apply-route.sh "$m" >/dev/null 2>&1 </dev/null &
		echo '{"ok":true}'
		;;
	restart)
	...
	zapret)
		...
		setsid /etc/tinyvless/apply-route.sh "${m:-selective}" >/dev/null 2>&1 </dev/null &
		echo "{\"zapret\":\"$2\"}"
		;;
	testlink)
