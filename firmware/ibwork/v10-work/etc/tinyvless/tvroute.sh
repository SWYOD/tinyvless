...
# discord → selective (обратная совместимость)
[ "$UDP_TUNNEL" = "discord" ] && UDP_TUNNEL=selective
...
for ch in pre_tcp pre_udp fwd_quic out_doh; do
...
if [ ! -f "$RU_MARKER" ]; then
	# TV_RU_SET=1 → грузим полную RU-базу (ru_cidr_compact.nft). TV_RU_SET=0 → пустой набор
	# (RAM~0), RU-роутинг только по доменам из direct_domains.
...
# Наборы доменов — всегда проверяем (могли отсутствовать после upgrade с v9).
nft list set ip tinyvless direct_domains >/dev/null 2>&1 || nft add set ip tinyvless direct_domains { type ipv4_addr\; } 2>/dev/null
nft list set ip tinyvless tunnel_domains >/dev/null 2>&1 || nft add set ip tinyvless tunnel_domains { type ipv4_addr\; } 2>/dev/null
nft list set ip tinyvless udp_tunnel_domains >/dev/null 2>&1 || nft add set ip tinyvless udp_tunnel_domains { type ipv4_addr\; } 2>/dev/null
...
add_bypass pre_udp
if [ "$UDP_TUNNEL" = "selective" ]; then
	# Селективный UDP: только домены из udp_tunnel_domains.list (+ Discord-порты как fallback).
	nft add rule ip tinyvless pre_udp ip daddr @udp_tunnel_domains meta l4proto udp tproxy to :$TPROXY_PORT meta mark set $MARK
	nft add rule ip tinyvless pre_udp udp dport { 19294-19344, 50000-65535 } tproxy to :$TPROXY_PORT meta mark set $MARK
elif [ "$UDP_TUNNEL" = "full" ]; then
...
echo "tvroute: mode=$MODE select=$SELECT_LEVEL ru_set=${TV_RU_SET:-1} udp=$UDP_TUNNEL tcp->redir:$REDIR_PORT udp->tproxy:$TPROXY_PORT quic->reject"
